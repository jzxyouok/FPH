...
$(function(){
	...
	//登录
	$('.J_sub_m_login').click(function(){
		var mobile      = $.trim($('#mobile').val());
		var password    = $.trim($('#password').val());
		var return_url  = unescape($.trim($('#url').val()));
		var url         = PINER.root + '/?g=m&m=user&a=login';
		var th          = $(this);
		if(return_url==''){
			layer.open({
				content: '参数出错',
	...
	$('.J_logout').click(function(){
		var url = PINER.root + '/?g=m&m=user&a=logout';
		layer.open({
			content: '你确定要退出登录?',
			btn: ['确认', '取消'],
			shadeClose: true,
	...
	//修改密码
	$('.J_m_edit_password').click(function(){
		var password     = $.trim($('#password').val());
		var newpassword  = $.trim($('#newpassword').val());
		var newpassword2 = $.trim($('#newpassword2').val());
		var url          = PINER.root + '/?g=m&m=user&a=password';
		var m_user_index = PINER.root + '/?g=m&m=user&a=index';
		var th           = $(this);
		if(password==''){
			layer.open({
				content: '请输入登录密码',
		...
		$.post(url,{password:password,newpassword:newpassword,newpassword2:newpassword2},function(result){
			if(result.status == 1){
				layer.open({
					content: result.msg,
					btn: ['确认'],
					shadeClose: true,
					yes: function(){
						window.location.href=m_user_index;
					}
				});
				return false;
			} else {
 ...
 //新建门店块
  ...
  stores_sc_ok.click(function(){
    ...
    company_c_url = PINER.root + '/?g=m&m=stores&a=insert';
    $.post(company_c_url, { get_city_id:get_city_id, get_one_company:get_one_company,stores_name:stores_name,contact:contact,contact_tel:contact_tel,stores_address:stores_address,type:2,
      ...
      if(result.status == 1){
        window.location.href= PINER.root + '/?g=m&m=stores&a=complete&type_ok=1&id='+result.data;
        return false;
      }
    },'json');
  });
  ...
  stores_c_ok.click(function(e){
    var stores_c_url   = PINER.root + '/?g=m&m=stores&a=ajax_check_name';
    var th = $(this);
    var stores_name      = $.trim($('#stores_name').val());
    if(stores_name==''){
		layer.open({
      ...
      if(result.status == 1){
		...
		layer.open({
			...
			yes: function(){
				...
				window.location.href= PINER.root + '/?g=m&m=stores&a=complete&type_ok=2&id='+result.data['result'];
			}
		});
        return false;
      }
...
//门店所属公司
  stores_c_c_ok.click(function(e){
    var company_c_url   =  PINER.root + '/?g=m&m=stores&a=ajax_check_company';
    var th = $(this);
    ...
    if(company_name==''){
     ...
     $.post(company_c_url,{company_name:company_name},function(result){
      if(result.status == 0){
        ...
        parent.next('.steps').fadeIn(300);
        //查找对应所属公司
        var company_cc_url = PINER.root + '/?g=m&m=stores&a=ajax_check_company_list';
        $.post(company_cc_url,
          {
...
$('.stores_get_add').click(function(){
	...
	var stores_c_url   =  PINER.root + '/?g=m&m=stores&a=ajax_check_code_id';
	...
	$.post(stores_c_url,{code_id:code_id},function(result){
			if(result.status == 0){//已存在
				layer.open({
					...
					shadeClose: false,
					yes: function(){
						window.location.href = PINER.root + '/?g=m&m=stores&a=store_required&id='+result.data['id'];
						return false;
					}
				});
                return false;
...
//门店日志信息录入
	$('.store_log').click(function(){
		var url = PINER.root + '/?g=m&m=stores&a=store_log_insert';
	...
	$('.get_project_log').click(function(){
		var url = PINER.root + '/?g=m&m=project&a=project_log_insert';
	...
	$('.get_project_contact').click(function(){
		var url = PINER.root + '/?g=m&m=project&a=ajax_project_contact_add';
		...
		$.post(url,{thework:thework,name:name,tel:tel,project_contact_id:project_contact_id,project_contact_cid:project_contact_cid},function(result){
			if(result.status == 1){
				...
				window.location.href = PINER.root + '/?g=m&m=project&a=project_detail&id='+result.data;
				return false;
            } else {
	...
	$('.stores_username_add').click(function(){
		...
		var url = PINER.root + '/?g=m&m=stores&a=ajax_stores_username_add';
		...
		$.post(url,{username:username,mobile:mobile,store_id:store_id},function(result){
			if(result.status == 1){
				...
				window.location.href = PINER.root + '/?g=m&m=stores&a=store_index&id='+result.data;
				return false;
            } else {
	...
	$('.project_contact_add_arr').click(function(){
		...
		var url = PINER.root + '/?g=m&m=project&a=ajax_project_contact_add_arr';
		...
		$.post(url,{title:title,address:address,str_thework:str_thework,str_name:str_name,str_tel:str_tel,property_cate_id:property_cate_id,item_price:item_price,get_city_id:get_city_id},function(result){
			if(result.status == 1){
				...
				window.location.href = PINER.root + '/?g=m&m=project&a=project_list';
				return false;
            } else {
...
//门店日志信息修改
	$('.edit_store').click(function(){
		var url = PINER.root + '/?g=m&m=stores&a=ajax_edit_store';
		var store_info      = $.trim($('.store_info').val());
		var store_id      = $.trim($('#store_id').val()); 
		var store_type      = $.trim($('#store_type').val()); 
		 ...
		 $.post(url,{store_info:store_info,store_id:store_id,store_type:store_type},function(result){
			if(result.status == 1){
				...
				window.location.href = PINER.root + '/?g=m&m=stores&a=store_info&id='+result.data;
				return false;
            } else {
            	window.location.href = PINER.root + '/?g=m&m=stores&a=store_info&id='+result.data;
				layer.open({
	...
	$('.J_add_case').click(function(){
		...
		var address     = $.trim($('#address').val());
		var url         = PINER.root + '/?g=m&m=case&a=add';
		var th          = $(this);
		if(name==''){
			layer.open({
				content: '请输出案场名称',
		...
		$.post(url,{name:name,city_id:city_id,address:address},function(result){
			if(result.status == 1){
				layer.open({
					...
					shadeClose: false,
					yes: function(){
						window.location.href = PINER.root + '/?g=m&m=case&a=detail&id='+result.data;
					}
				});
				return false;
            }else{
	...
	$('.J_case_user').live('click',function(){
		...
		var url      = PINER.root + '/?g=m&m=case&a=detail';
		var th       = $(this);
		if(stores_id==''){
			layer.open({
				content: '系统参数错误',
...
function shai_stores_city_pid(){
   if($('#shai_city').val() == ''){
    var url = PINER.root + '/?g=m&m=stores&a=shai_city';
    $.post(url,{
