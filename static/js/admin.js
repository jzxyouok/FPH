...
;$(function($){
	...
	$('.J_showdialog').live('click', function(){
		...
		var select_url= $("#select_area select:nth-child(3)").attr('data-pid');
		if(!isNaN(select_url))
		{
			duri = duri+'&city_id='+select_url+'';
		}
		else
		{
			duri = duri+'&city_id=0';
		...
		switch(ftype){
			case 'image':
				if(!$(this).find('.attachment_tip')[0]){
					$('<div class="attachment_tip"><img src="'+rel+'" width="200" /></div>').prependTo($(this)).fadeIn();
				}else{
...
;(function($){
    //联动菜单
    $.fn.city_select = function(options) {
		var newId=0;
        var settings = {
            field: 'J_city_id',
			buffer:'J_city_buffer',
            top_option: lang.please_select
        };
        if(options) {
            $.extend(settings, options);
        ...
        self.nextAll('.J_city_select').remove();
        $('<option value="">--'+settings.top_option+'--</option>').appendTo(self);
        $.getJSON(uri, {id:pid}, function(result){
            if(result.status == '1'){
                for(var i=0; i<result.data.length; i++){
        ...
        var j = 1;
        $('.J_city_select').die('change').live('change', function(){
            var _this = $(this),
            _pid = _this.val();
            _this.nextAll('.J_city_select').remove();
            if(_pid != ''){
                $.getJSON(uri, {id:_pid}, function(result){
                    if(result.status == '1'){
                        var _childs = $('<select class="J_city_select mr10" data-pid="'+_pid+'"><option value="">--'+settings.top_option+'--</option></select>')
                        for(var i=0; i<result.data.length; i++){
                            $('<option value="'+result.data[i].id+'">'+result.data[i].name+'</option>').appendTo(_childs);
                        }
                        _childs.insertAfter(_this);
			...
			if($(this).val()!=''){
				newId=$(this).siblings('select').length;
			}
			else{
				newId=$(this).siblings('select').length-1;
			}
			$('#'+settings.buffer).val(newId);
        });
    }


    //联动菜单2
    $.fn.city_select2 = function(options) {
        var settings = {
            field: 'J_city_id2',
            top_option: lang.please_select
        };
        ...
        self.nextAll('.J_city_select2').remove();
        $('<option value="">--'+settings.top_option+'--</option>').appendTo(self);
        $.getJSON(uri, {id:pid}, function(result){
            if(result.status == '1'){
                for(var i=0; i<result.data.length; i++){
        ...
        var j = 1;
        $('.J_city_select2').die('change').live('change', function(){
            var _this = $(this),
            _pid = _this.val();
            _this.nextAll('.J_city_select2').remove();
            if(_pid != ''){
                $.getJSON(uri, {id:_pid}, function(result){
                    if(result.status == '1'){
                        var _childs = $('<select class="J_city_select2 mr10" data-pid="'+_pid+'"><option value="">--'+settings.top_option+'--</option></select>')
                        for(var i=0; i<result.data.length; i++){
                            $('<option value="'+result.data[i].id+'">'+result.data[i].name+'</option>').appendTo(_childs);
                        }
                        _childs.insertAfter(_this);
