...
*/
		...
		//批量操作用户*属性结构
		$('input[data-tdtype="user_batch_action"]').live('click', function() {
			var btn = this;
			var checkitem_length = $('.J_checkitem:checked').length;
			
			if(checkitem_length == 0){
                $.pinphp.tip({content:'请现在要转移的经纪人', icon:'alert'});
				return false;
            }
			...
			if(ids != ''){
				...
				$.pinphp.tip({content:'请选择要转移的经纪人', icon:'alert'});
				return false;
				action_user();
			}
