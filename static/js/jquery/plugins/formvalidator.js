...
var regexEnum = 
{
	...
	intege2:"^-[1-9]\\d*$",					//负整数
	num:"^([+-]?)\\d*\\.?\\d+$",			//数字
	price1:"^[1-9]\\d{0,5}(\\.\\d{1,2})?$",　　	//价格
	price2:"^([1-9]\\d+|\\d)(\\.\\d{1,2})?$",　　	//可以为0，两位小
	num1:"^[1-9]\\d*$",					//正数（正整数）
	num2:"^-[1-9]\\d*|0$",					//负数（负整数 + 0）
    num3:"^([1-9]\\d*)(\\.\\d{1,2})?$",
	num4:"^[1-9]\\d{0,3}$|^[1-5]\\d{4}$|^6[0-4]\\d{3}$|^65[0-4]\\d{2}$|^655[0-2]\\d{1}$|^6553[0-2]$",
	num5:"^[1-65532]$",
	num6:"^[1-9]\\d*|0$",					//正数（正整数 + 0）
	num7:"^([1-9]\\d*(\\d+)?|0)$",					//正数（正整数 + 0）
	decmal:"^([+-]?)\\d*\\.\\d+$",			//浮点数
	...
	zipcode:"^\\d{6}$",						//邮编
	mobile:"^13[0-9]{1}[0-9]{8}$|15[012356789]{1}[0-9]{8}$|18[0123456789]{1}[0-9]{8}$|14[57]{1}[0-9]$|17[0123456789]{1}[0-9]{8}$",				//手机
	...
	username:"^([\u4E00-\uFA29]|[\uE7C7-\uE7F3]|[a-zA-Z])*$",						//用来用户注册。匹配由数字、26个英文字母或者下划线组成的字符串
	letter:"^[A-Za-z]+$",					//字母
	letter_u:"^[A-Z]+$",					//大写字母
	letter_l:"^[a-z]+$",					//小写字母
	idcard:"^[1-9]([0-9]{14}|[0-9]{17})$",	//身份证
	ps_username:"^[\\u4E00-\\u9FA5\\uF900-\\uFA2D_\\w]+$", //中文、字母、数字 _
	letterNum:"^[A-Za-z0-9]+$",					//字母
	gtTwo:"^[2-9]$|^[1-9]\\d+$" //大于等于2的数字
}
...
/*绑定checkbox与input的验证关系函数
 @opt.checkId:checkbox的id
 @opt.validInputId:需要验证的inputId
 @opt.checkValid:选中时的验证函数
 @opt.passValid:没选中时得验证函数
 * */
function bindCheck(opt){
	var oCheckBox=$(opt.checkId),oInput=$(opt.validInputId);
	checkVal();
	oCheckBox.on('change',checkVal);

	function checkVal(){
		if(oCheckBox.is(':checked')){
			opt.checkValid && opt.checkValid.call(oInput);
		}else{
			opt.passValid && opt.passValid.call(oInput);
		}
	}
}
