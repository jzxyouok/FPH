<?php
class vote_mengbao1Action extends frontendAction {
	...
	*/
    ...
    public function view(){
            ...
            if(empty($openid)){
                    ...
                    if(!$code){
                    $url='http://www.fangpinhui.com/?g=weixin&m=vote_mengbao1&a=detail&id='.$id;
    ...
    public function vote(){
            ...
            if(empty($openid)){
                  $code = $_GET["code"];
                if(!$code){
                      $url='http://www.fangpinhui.com/?g=weixin&m=vote_mengbao1&a=vote';
    ...
    public function join(){
            ...
            $code = $_GET["code"];
            if(!$code){
            	$url='http://www.fangpinhui.com/?g=weixin&m=vote_mengbao1&a=join';
            	redirect('https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$appid.'&redirect_uri='.urlencode($url).'&response_type=code&scope=snsapi_base&state=123#wechat_redirect');
            	exit;
            }
                $get_token_url = 'https://api.weixin.qq.com/sns/oauth2/access_token?appid='.$appid.'&secret='.$appsecret.'&code='.$code.'&grant_type=authorization_code';
...
$result = array('status'=>0,'msg'=>$openid,'url'=>$url);exit(json_encode($result));
	...
	if(empty($openid)){
	   ...
	   if($data['avatar']){
			...
			$this->redirect(u('vote_mengbao1/voteError',array('msg'=>'请上传图片')));exit;
		}
        $imgstr = '';
		...
		*/
		if(empty($data['tel'])){
			$this->redirect(u('vote_mengbao1/voteError',array('msg'=>'手机号不能为空')));
            exit;
		}     
        ...
        if($isjoin){
            $this->redirect(u('vote_mengbao1/voteError',array('msg'=>'已经报名了！')));
            exit;
        }else{
            $back_id = M('vote_young')->data($data)->add();            
            $this->redirect(u('vote_mengbao1/voteSuccess',array('back_id'=>$back_id)));
            exit;   
        }
    }
