<?php
class stores_countAction extends weixin_userbaseAction {
	...
	public function index(){
		...
		$l_date = $this->_mod_l->where('uid='.$uid)->order('id asc')->getField('add_time');
		$s_date = $this->_mod_s->where('uid='.$uid)->order('id asc')->getField('add_time');
		if($l_date>$s_date){
			$date = $s_date;//最小日期
			if($date==0){
				$date = $l_date;//最小日期
		...
		foreach($arr_hours as $key=>$val){
			//拜访门店信息 stores_log
			$arr_hours[$key]['l_list'] = $this->_mod_l->field("id,pid,add_time")->group('pid')->where("uid=".$uid." AND add_time between ".strtotime($val['date'].' 00:00:00')." and ".strtotime($val['date'].' 23:59:59')."")->select();
			...
			//拓展门店信息
			$arr_hours[$key]['s_list'] = $this->_mod_s->field("id,name,add_time")->where("uid=".$uid." AND add_time between ".strtotime($val['date'].' 00:00:00')." and ".strtotime($val['date'].'23:59:59')."")->select();
	...
	public function week(){
		...
		foreach($arr_hours as $key=>$val){
			if($key%7 ==0){
				...
				//拜访门店信息 stores_log
		 		$arr_week[$key]['l_count'] += count($this->_mod_l->group('pid')->where("uid=".$uid." AND add_time between ".strtotime($tim.' 00:00:00')." and ".strtotime($val['date'].' 23:59:59')."")->select());
		 		//拓展门店信息
		 		$arr_week[$key]['s_count'] += count($this->_mod_s->where("uid=".$uid." AND add_time between ".strtotime($tim.' 00:00:00')." and ".strtotime($val['date'].'23:59:59')."")->select());
		 		$arr_week[$key]['s_list'] = $this->_mod_s->field('id')->where("uid=".$uid." AND add_time between ".strtotime($tim.' 00:00:00')." and ".strtotime($val['date'].'23:59:59')."")->select();
	...
	public function month(){
		...
		foreach($arr_hours as $key=>$val){
			if($key%31 == 0){
				...
				//拜访门店信息 stores_log
		 		$arr_month[$key]['l_count'] += count($this->_mod_l->group('pid')->where("uid=".$uid." AND add_time between ".strtotime($tim.' 00:00:00')." and ".strtotime($val['date'].' 23:59:59')."")->select());
		 		
		 		//拓展门店信息
		 		$arr_month[$key]['s_count'] += count($this->_mod_s->where("uid=".$uid." AND add_time between ".strtotime($tim.' 00:00:00')." and ".strtotime($val['date'].'23:59:59')."")->select());
		 		$arr_month[$key]['s_list'] = $this->_mod_s->field('id')->where("uid=".$uid." AND add_time between ".strtotime($tim.' 00:00:00')." and ".strtotime($val['date'].'23:59:59')."")->select();
		 		//经纪人数量
