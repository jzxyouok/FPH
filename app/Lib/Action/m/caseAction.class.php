<?php
class caseAction extends m_userbaseAction {
	...
	public function ajax_store_index(){
        ...
        foreach($list as $k=>$v){
        	...
        	$str .='<li><a href="'.U('m/case/detail',array('id'=>$v['id'])).'"><h2>'.$v['name'].'<b class="LBL_STATUS sc_11">邀请码:'.$v['code_id'].'</b></h2><span class="address">'.$v['address'].'</span><span class="store_code">服务专员:'.$list[$k]['username'].' '.$list[$k]['mobile'].'</span><span class="store_code_r">成员'.$list[$k]['user_count'].'人</span><i></i></a></li>';
        }
        if($str){
	...
	public function add(){
		...
		if(IS_POST){
			...
			//用户信息
			$admin_info = D('admin')->admin_info($uid);
			$data['mid']         = $uid;
			$data['pid']         = 420;
			...
			$data['contact']     = $admin_info['username'];
			$data['contact_tel'] = $admin_info['mobile'];
			$data['sid']     	 = $uid;
			$data['add_time']    = time();
			$data['status']      = 1;
	...
	public function detail(){
		...
		$stores_info = M('stores')->field('id,name,address,status,img,mid,service,code_id,sid')->where('id='.$id)->find();
