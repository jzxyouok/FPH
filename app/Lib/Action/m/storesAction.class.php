<?php
class storesAction extends m_userbaseAction {
	...
	public function store_list(){
		...
		foreach($list as $key=>$value){
			if($value['sid']!=0){
				$user_info = M('admin')->field('username,mobile')->where('id='.$value['sid'])->find();
		...
		$count = $this->_mod_s->where($where)->count('id');
		//所在城市 门店所有成员数
		$store_count = 0;
	...
	public function store_show(){
		...
		$stores_info = array();
		$user_stores_id = $id;
		$stores_info = M('stores')->field('id,name,code_id,address,status,contact,contact_tel,service,img,mid,sid')->where('id='.$user_stores_id)->find();
		//$stores_info['user_info'] = M('user')->field('username,mobile')->where(array('id'=>$uid))->find();
		if($stores_info['sid']!=0){
			$stores_info['user_info'] = M('admin')->field('username,mobile')->where(array('id'=>$stores_info['sid']))->find();
		}else{
			$stores_info['user_info'] = M('user')->field('username,mobile')->where(array('id'=>$stores_info['mid']))->find();
		}
		$stores_info['storesid'] = $user_stores_id;
		$stores_log_list = M('stores_log')->where('pid='.$id)->field('id,mid,info,add_time')->order('id DESC')->limit(0,5)->select();
		foreach($stores_log_list as $k=>$v){
			$stores_log_list[$k]['name'] =  M('admin')->where(array('id'=>$v['mid']))->getField('username');
			$stores_log_list[$k]['mobile'] =  M('admin')->where(array('id'=>$v['mid']))->getField('mobile');
		}
		$fph  = C('DB_PREFIX');
	...
	public function ajax_store_log_list(){
		...
		$start = $page*5; 
		$stores_log_list = M('stores_log')->where('pid='.$id)->field('id,mid,info,add_time')->order('id DESC')->limit($start,5)->select();
		$str = '';
		foreach($stores_log_list as $k=>$v){
			$stores_log_list[$k]['name'] =  M('admin')->where(array('id'=>$v['mid']))->getField('username');
			$stores_log_list[$k]['mobile'] =  M('admin')->where(array('id'=>$v['mid']))->getField('mobile');
	...
	public function ajax_company_list(){
        ...
        foreach($list as $k=>$v){
        	if($v['sid']!=0){
				$user_info = M('admin')->field('username,mobile')->where('id='.$v['sid'])->find();
				$list[$k]['username'] = $user_info['username'];
				$list[$k]['mobile'] = $user_info['mobile'];
			}
			$list[$k]['user_count'] = M('user')->where('stores_id='.$v['id'])->count('id');
        	 $str .='<li><a href="'.U('m/stores/store_show',array('id'=>$v['id'])).'"><h2>'.$v['name'].'<b class="LBL_STATUS sc_11">邀请码：'.$v['code_id'].'</b></h2><span class="address">'.$v['address'].'</span><span class="store_code">服务专员:'.$list[$k]['username'].' '.$list[$k]['mobile'].'</span><span class="store_code_r">成员'.$list[$k]['user_count'].'人</span><i></i></a></li>';
        }
        if($str){
		    $this->ajaxReturn(1,'',$str);
	    }else{
	...
	public function index(){
		...
		if($id){
			$user_stores_id = $id;
			$uid = M('stores')->where('id='.$id)->getField('mid');
		}
		if($user_stores_id!=0){
			$stores_info = M('stores')->field('id,name,mid,code_id,type,address,status,contact,contact_tel,service,img,sid')->where('id='.$user_stores_id)->find();
			$stores_info['storesid'] = $user_stores_id;
		...
		if($stores_info['type']==1){
			$stores_type_info = M('admin')->field('username,mobile')->where('id='.$stores_info['sid'])->find();
	...
	public function store_info(){
		...
		$stores_info = M('stores')->field('id,name,code_id,address,status,contact,contact_tel,service,pid,city_id,img,sid')->where('id='.$id)->find();
	...
	public function upload_avatar() {
        ...
        if (!empty($_FILES['avatar']['name'])) {
			...
			$result = $fdfs_obj->fdfs_upload('avatar','100','70','_100x70',false);
			if($result){
				$savename = $result['group_name'].'/'.$result['filename'];
				M('stores')->where(array('id'=>$id))->save(array('img'=>$savename));
				$arr_pic = explode('.', $savename);
                $data = C('img_url').$arr_pic[0].'_100x70.'.$arr_pic[1];
                $this->ajaxReturn(1, L('upload_success'), $data);
			}else{
				 $this->ajaxReturn(0, '上传图片出错');
			}
			
           /* $result = $this->_upload($_FILES['avatar'], 'stores_avatar/'. $dir, array('width'=>'100','height'=>'70','suffix'=>trim($suffix, ',')));
            if ($result['error']) {
    ...
    public function ajax_edit_store(){
    	...
    	if($store_type ==1){//联系人
     	...
     	//门店日志表录入
       	D('stores_log')->insert_m($uid,$id,time(),$info);
    	if ($id) {
            $this->ajaxReturn(1, '操作成功',$store_id);
        } else {
    ...
    public function insert(){
		...
		$data_s['type'] = 2;
       	...
       	if(empty($s_name)){
	       	...
	       	//门店日志表录入
	       	$info = '创建了本门店';
	       	D('stores_log')->insert_m($uid,$lastid_s,$tim,$info);
       	}else{
    ...
    public function store_log_insert(){
    	...
    	if ($lastid_log) {
    		$user_info = M('admin')->field('username,mobile')->where(array('id'=>$uid))->find();
