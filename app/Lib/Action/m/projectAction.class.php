 ...
 * @author lishun
 */
class projectAction extends m_userbaseAction {
	...
	public function project_list(){
		...
		$search['count'] = $this->_mod_p->where($where)->count('id');
		//区域显示
	...
	public function ajax_project_list(){
		...
		$str = '';
		foreach($list as $k=>$v){
             $str .='<li><a href="'.U('m/project/project_detail',array('id'=>$v['id'])).'"><h2>'.$v['title'].'</h2><span class="address">'.$v['address'].'</span><i></i></a></li>';
		}
		if($str){
	...
	public function project_detail(){
		...
		//发布跟进
		$project_log_list = M('property_log')->where('pid='.$id)->field('id,pid,mid,info,add_time')->order('id DESC')->limit(0,5)->select();
		foreach($project_log_list as $k=>$v){
			$project_log_list[$k]['name'] =  M('admin')->where(array('id'=>$v['mid']))->getField('username');
			$project_log_list[$k]['mobile'] =  M('admin')->where(array('id'=>$v['mid']))->getField('mobile');
		}
		$fph  = C('DB_PREFIX');
	...
	public function ajax_project_log_list(){
		...
		$page = $this->_post('page','intval');
		$start = $page*5; 
		$project_log_list = M('property_log')->where('pid='.$id)->field('id,pid,mid,info,add_time')->order('id DESC')->limit($start,5)->select();
		$str = '';
		foreach($project_log_list as $k=>$v){
			$project_log_list[$k]['name'] =  M('admin')->where(array('id'=>$v['mid']))->getField('username');
			$project_log_list[$k]['mobile'] =  M('admin')->where(array('id'=>$v['mid']))->getField('mobile');
