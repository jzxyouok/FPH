<?php
class app_push_cateAction extends backendAction {
    public function _initialize() {
        parent::_initialize();
        $this->_mod = D('app_push_cate');
    }
    ...
    public function index() {
        ...
        $array = array();
        foreach($result as $r) {
            $r['str_img'] = $r['img'] ? '<div class="img_border"><img src="'.attach($r['img'], 'app_push_cate').'" width="26" height="26" class="J_preview" data-bimg="'.attach($r['img'], 'app_push_cate').'"/></div>' : '';
            ...
            $r['str_manage'] = '<a href="javascript:;" class="J_showdialog" data-uri="'.U('app_push_cate/add',array('pid'=>$r['id'])).'" data-title="'.L('add_article_cate').'" data-id="add" data-width="500" data-height="">'.L('add_article_subcate').'</a> |
                                <a href="javascript:;" class="J_showdialog" data-uri="'.U('app_push_cate/edit',array('id'=>$r['id'])).'" data-title="'.L('edit').' - '. $r['name'] .'" data-id="edit" data-width="500" data-height="">'.L('edit').'</a> |
                                <a href="javascript:;" data-acttype="ajax" class="J_confirmurl" data-uri="'.U('app_push_cate/delete',array('id'=>$r['id'])).'" data-msg="'.sprintf(L('confirm_delete_one'),$r['name']).'">'.L('delete').'</a>';
