<?php
class pringles_cateAction extends backendAction {
    public function _initialize() {
        parent::_initialize();
        $this->_mod = D('pringles_cate');
    }
    ...
    public function index() {
        ...
        foreach($result as $r) {
            ...
            $r['str_manage'] = '<a href="javascript:;" class="J_showdialog" data-uri="'.U('pringles_cate/add',array('pid'=>$r['id'])).'" data-title="'.L('add_article_cate').'" data-id="add" data-width="500" data-height="">'.L('add_article_subcate').'</a> |
                                <a href="javascript:;" class="J_showdialog" data-uri="'.U('pringles_cate/edit',array('id'=>$r['id'])).'" data-title="'.L('edit').' - '. $r['name'] .'" data-id="edit" data-width="500" data-height="">'.L('edit').'</a> |
                                <a href="javascript:;" data-acttype="ajax" class="J_confirmurl" data-uri="'.U('pringles_cate/delete',array('id'=>$r['id'])).'" data-msg="'.sprintf(L('confirm_delete_one'),$r['name']).'">'.L('delete').'</a>';
    ...
    protected function _before_insert($data = '') {
    	//设置唯一键
    	$data['unique_key']	= $this->_mod->get_unique_key( $data['name'], $data['pid'] );
    	if( $data['unique_key'] == FALSE )	$this->ajaxReturn(0, L('article_cate_already_exists'));
    ...
    protected function _before_update($data = '') {
    	//设置唯一键
    	$data['unique_key']	= $this->_mod->get_unique_key( $data['name'], $data['pid'], $data['id'] );
    	if( $data['unique_key'] == FALSE )	$this->ajaxReturn(0, L('article_cate_already_exists'));
	...
	public function _before_delete(){
		...
		M('property_cate')->where(array('pid'=>$id))->delete();
	}
