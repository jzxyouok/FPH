<?php
class property_group_buyAction extends backendAction
{
    ...
    */
    public function index() {
        ...
        //$redis->handler->hmset('key2',array('field1'=>'v1','field2'=>'v2'));
        //$aa = $redis->handler->hmget('activity.kanjia.5',array('now_number'));//单个字段读取
        //$aa = $redis->handler->hgetall('activity.kanjia.5');//全部读取
        //print_r($aa);
        ...
        $where ='pid = '.$pid;
        $count = M('group_buy')->where($where)->count('id');
        import("ORG.Util.Page");
        $p = new Page($count, 20);
        $page = $p->show();
        $list = M('group_buy')->field('id,time_start,time_end,add_time,demand,preferential,status,schedule')->where($where)->order('id DESC')->limit($p->firstRow.','.$p->listRows)->select();
        foreach($list as $key=>$val){
            $list[$key]['count'] = M('join_group_buy')->where(array('gid'=>$val['id'],'status'=>1))->count('id');
        }
   ...
   */
    public function edit() {
        ...
        if(IS_POST){
            ...
            if($status){
                $activity_count = M('group_buy')->where('pid = '.$id.' AND status = 1 AND id !='.$pid.'')->count('id');
                if($activity_count){
            ...
            $data['id']              = $pid;
            $data['add_time']   = time();
            if(false !== M('group_buy')->where('id = '.$pid)->save($data)){
                $now_number = M('join_group_buy')->where('gid = '. $pid)->count('id');//单个字段读取
                $data['now_number'] = $now_number;
                $redis = new CacheRedis($this->RedisDataBase);
                $redis->handler->hmset('activity.kanjia.'.$pid.'',$data);
        ...
        $info = M('group_buy')->where('id='.$pid)->find();
   ...
   */
    public function add() {
        ...
        if(IS_POST){
            ...
            if($status){
                $activity_count = M('group_buy')->where('pid = '.$id.' AND status = 1')->count('id');
                if($activity_count){
                    $this->error('系统中已经存在该楼盘的有效活动');
                }
            }
            ...
            $data['status']        = $status;
            $data['add_time']   = time();
            if($return_id= D('group_buy')->add($data)){
                $data['now_number'] = 0;
                $data['id'] = $return_id;
                $redis = new CacheRedis($this->RedisDataBase);
                $redis->handler->hmset('activity.kanjia.'.$return_id.'',$data);
 ...
 */
    public function lists(){
        ...
        $p = new Page($count, 20);
        $page = $p->show();
        $list = M('join_group_buy')->field('id,gid,uid,add_time,status')->where($where)->order('id DESC')->limit($p->firstRow.','.$p->listRows)->select();
        foreach($list as $key=>$val){
            M(NULL,'fph_',C('DB_member'));
            $list[$key]['username'] = M('member_extend')->where(array('uid'=>$val['uid']))->getfield('username');
            $list[$key]['mobile'] = M('member')->where(array('id'=>$val['uid']))->getfield('mobile');
        }
        $this->assign('list',$list);
        $this->assign('page',$page);
    ...
    public function groupBuyIndexImport(){
        ...
        //设置标题
        $rowVal = array(0=>'编号',1=>'楼盘名称', 2=>'城市', 3=>'有效期', 4=>'优惠幅度', 5=>'要求人数', 6=>'参与人数', 7=>'砍价状态');
        ...
        $objPhpExcel->getActiveSheet()->setCellValue('H1', '砍价状态');
    ...
    //报表中心*导出砍价客户
    public function groupBuyListImport(){
        M('join_group_buy','fph_',C('DB_activity'));
        $field = 'id,gid,uid,status,add_time';
        $order = 'add_time DESC';
        $page  = 200;
        $list = D('join_group_buy')->lists($field, $order, $page);
