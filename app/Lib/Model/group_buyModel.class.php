<?php
class group_buyModel extends RelationModel
{
    public function lists($field, $order, $page=20) {
        ...
        $list = M('group_buy')->field($field)->where($where)->order($order)->limit($p->firstRow.','.$p->listRows)->select();
        ...
        foreach($list as $key=>$val){
            ...
            $list[$key]['count'] = D('join_group_buy')->joinGroupBuyCount($joinWhere);
    ...
    public function getFind($field, $where) {
        $data = M('group_buy')->field($field)->where($where)->find();
        return $data;
    }
}