<?php
class pringles_cateModel extends Model
{
    /**
     * 生成spid 
     * 
     ...
     * @param unknown $pid
     */
    public function get_unique_key( $name, $pid, $id=0  )
    {
    	$where			= "id='{$pid}'";
    	$parent_name	= $this->where($where)->getField('name');
    	$unique_key		= md5($parent_name.$name);
    	$where			= "unique_key='{$unique_key}' AND id <> '{$id}'";
    	return $this->where($where)->count('id') ? FALSE : $unique_key;
    }
