...
class WeiXin {
  ...
  public function handleEvent(){
  	if($this->isEventSubscribe()){  //是否为普通关注事件
  	...
  	}else if($this->isEventScanSubscript()){  //是否为扫描二维码关注事件
  		
  	}else if($this->isEventScan()){  //是否为已关注扫描二维码事件
  	...
  	if($id){ //如有关键字
  		...
  		switch ($row_w['pid']){
  			...
  			case 3://单图文回复
  				...
  				$title[0]['PicUrl']= "http://www.fangpinhui.com/data/upload/advert/".$row_w['img']."";
  				$title[0]['Description']   = "".$row_w['intro']."";
  				$title[0]['Url']   = "".$row_w['url']."";
  			...
  			case 4://多图文回复
  				...
  				for($i = 0; $i < count($titlet); $i++){
  					...
  					$title[$i]['PicUrl']= "http://www.fangpinhui.com/data/upload/advert/".$img[$i]."";
  					$title[$i]['Url']   = "".$url[$i]."";
  				}
  				$this->responseNewsMessage($title);
  				break;
  		...
  		$this->responseTextMessage("问题已收到！等不及就微信添加好友FPHhome，加她的时候注明是哪个区域版块的经纪人哦！");
  	}
  }
