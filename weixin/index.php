...
/**
  ...
  * author H.J.H
  * date 2014/11/21
  * linkme QQ1481031592 keedoo.net
  */

header("Content-type: text/html; charset=utf-8");
require "./conn.php";
//require "./menu_display.php";绝对不能再引用此文件，让他单独运行

new WeiXin('fangpinhui', $appid = null, $appsecret = null, $debug = false);
...
class WeiXin {
  ...
  public function __construct($token, $appid = null, $appsecret = null, $debug = false) {
    $this->token = $token; 
    isset($_GET['echostr']) ? $this->checkSignature() : $this->handleRequest(); //签名验证请求中，url中有个echostr变量，而在响应消息中是没有的
  }
  ...
  private function checkSignature() {
    ...
    $tmpStr = sha1($tmpStr);   //计算字符串的 SHA-1 散列         40字符十六进制数
  ...
  private function handleRequest() {
      $this->postStr = $GLOBALS["HTTP_RAW_POST_DATA"];    //和$_POST是一样的，但post过来的数据不是PHP能够识别的。比如 text/xml 或者 soap
      if (!empty($this->postStr)) {   //若变量已存在、非空字符串或者非零，则返回 false 值
        $this->postObj = simplexml_load_string($this->postStr, 'SimpleXMLElement', LIBXML_NOCDATA);  //把 XML 字符串载入对象中
		$MsgType       = $this->getMsgType();//获取消息类型
		switch($MsgType){
			case "text"://文本消息
				$this->handleText();
				break;
  ...
  public function handleText() {
	$keyword = $this->requestText();
	
	/*多客服开始*/
	/*if (strstr($keyword, "您好") || strstr($keyword, "你好") || strstr($keyword, "在吗") || strstr($keyword, "有人吗")){
		$result = $this->transmitService($this->postObj);
		echo $result;
    }*/
	/*多客服结束*/
	//查找手机号码是否有优惠券
	if(is_numeric($keyword) && strlen($keyword)==4){
		$sql="SELECT id FROM fph_coupon WHERE mobile='".$keyword."' AND status=1";
		$res=mysql_query($sql);
		$rs=mysql_fetch_array($res,MYSQL_BOTH);
	}
	if($rs['id']){
	 	$sql="SELECT pid FROM fph_weixin_keyword_reply WHERE keyword='优惠券' AND status=1";
		$res=mysql_query($sql);
		$row=mysql_fetch_array($res,MYSQL_BOTH);
		$this->replyType($row['pid'],$rs['id']);
	}else{
		$sql     = "SELECT keyword,pid FROM fph_weixin_keyword_reply WHERE status=1 order by id DESC LIMIT 0,100";//查询数据库中有无提交的关键字
		$res     = mysql_query($sql);
		while($row=mysql_fetch_array($res,MYSQL_BOTH)){
			if(substr_count($row['keyword'], $keyword)){ //查看有木有关键词  以出现次数做判断
				$pid = $row['pid'];
				break;
			}
		}
  ...
  public function handleImage(){
    ...
    $sql="SELECT pid FROM fph_weixin_keyword_reply WHERE keyword='图片回复' AND status=1";
    $res=mysql_query($sql);
    $row=mysql_fetch_array($res,MYSQL_BOTH);
    	
    $this->replyType($row['pid']);
  }
  /**
   * 位置请求处理
   * @param $this->postObj
   ...
   */
   //自动获取
	public function handleLocation_auto(){
		//$location   = $this->requestLocation();
		//$contentStr = '房品汇提示您所处位置是:坐标为 X：'.$this->postObj->Latitude.'Y：'.$this->postObj->Longitude;
    $time       = time();
    $fromUsername = $this->getFromUserName();
    $Latitude = $this->postObj->Latitude;
    $Longitude = $this->postObj->Longitude;

    $search = "SELECT openid FROM fph_user_location WHERE openid = '$fromUsername'";
    $query=mysql_query($search);
    $row=mysql_fetch_array($query,MYSQL_BOTH);

    if(!$row['openid']){//判断为初次进入写入数据库
      $sql="insert into fph_user_location(openid,latitude,longitude,add_time) values('$fromUsername','$Latitude','$Longitude','$time')";
      mysql_query($sql);
    }else{//判断为二次进入，修改状态并更新时间
      $sql = "update fph_user_location set add_time = '$time',latitude = '$Latitude',Longitude = '$Longitude' where openid = '$fromUsername'";
      mysql_query($sql);
    }


		//$this->responseTextMessage($contentStr);
	}
	//发送获取
  public function handleLocation(){
  	$location   = $this->requestLocation();
  	$contentStr = '房品汇提示您所处位置是:'.$location['Label'].'坐标为 X：'.$location['Location_X'].'Y：'.$location['Location_Y'];
  	$this->responseTextMessage($contentStr);
  }
  /**
   * 链接请求处理
  ...
  public function handleLink(){
    ...
    $sql="SELECT pid FROM fph_weixin_keyword_reply WHERE keyword='链接回复' AND status=1";
    $res=mysql_query($sql);
    $row=mysql_fetch_array($res,MYSQL_BOTH);
  ...
  public function handleVoice(){
  	$voice       = $this->requestVoice();
  	
    $sql="SELECT pid FROM fph_weixin_keyword_reply WHERE keyword='语音回复' AND status=1";
    $res=mysql_query($sql);
    $row=mysql_fetch_array($res,MYSQL_BOTH);
  ...
  public function handleVideo(){
    ...
    $sql="SELECT pid FROM fph_weixin_keyword_reply WHERE keyword='视频回复' AND status=1";
    $res=mysql_query($sql);
    $row=mysql_fetch_array($res,MYSQL_BOTH);
  ...
  public function handleEvent(){
  	if($this->isEventSubscribe()){  //是否关注事件
  		
  		$fromUsername = $this->getFromUserName();
		if(isset($this->postObj->EventKey) && !empty($this->postObj->EventKey)){//扫面带参数的二维码
     
      $admin_id = explode('_',$this->postObj->EventKey);
	    $sql="SELECT pid FROM fph_weixin_keyword_reply WHERE id=32 AND status=1";
      $res=mysql_query($sql);
      $row=mysql_fetch_array($res,MYSQL_BOTH);
      $this->replyType($row['pid'],$admin_id[1]);
			//$this->responseTextMessage("你是通过扫码关注".$this->postObj->EventKey);
		}else{
			//用户信息写入到数据库
			$time       = time();
			$search = "SELECT id FROM fph_weixin_user WHERE status = 0 and fromusername = '$fromUsername'";
			$query=mysql_query($search);
			$row_gf=mysql_fetch_array($query,MYSQL_BOTH);
			if(!$row_gf['id']){//判断为初次关注写入数据库
				$status     = 1;
				$sql="insert into fph_weixin_user(fromusername,add_time,status) values('$fromUsername','$time','$status')";
				mysql_query($sql);
			}else{//判断为二次关注，修改状态并更新时间
				$sql = "update fph_weixin_user set add_time = '$time',status = 1 where fromusername = '$fromUsername'";
				$result = mysql_query($sql);
			}
			//关注后推送相关消息
			$sql_g="SELECT pid FROM fph_weixin_guanzhu_reply WHERE status=1 order by id DESC LIMIT 1";
			$res_g=mysql_query($sql_g);
			$row_g=mysql_fetch_array($res_g,MYSQL_BOTH);
				
			$this->replyType($row_g['pid']);
		}
  		
  		
  	}elseif($this->postObj->Event=="SCAN"){//已经关注  扫描了带参数的二维码

      $admin_id = $this->postObj->EventKey;
      $sql="SELECT pid FROM fph_weixin_keyword_reply WHERE id=32 AND status=1";
      $res=mysql_query($sql);
      $row=mysql_fetch_array($res,MYSQL_BOTH);
      $this->replyType($row['pid'],$admin_id);
			//$this->responseTextMessage("你是已经关注过了，扫描了带参数的二维码".$this->postObj->EventKey.$this->getFromUserName());
	}else if($this->isEventUnSubscribe()){  //是否为取消关注事件
  		//修改粉丝关注状态
  		$fromUsername = $this->getFromUserName();
  		$sql = "update fph_weixin_user set status = 0 where fromusername = '$fromUsername'";
  		$result = mysql_query($sql);
  		
  	}else if($this->isEventLocation()){  //是否为上报地理位置事件
  		
		///$time       = time();
		//$status     = 1;
		//$fromUsername = $this->getFromUserName();
		//$sql="insert into fph_weixin_locat(fromUsername,add_time,status) values('$fromUsername','$time','$status')";
		//mysql_query($sql);
		
		$this->handleLocation_auto();
  	}else if($this->isEventClick()){   //是否为菜单点击事件
  		         
  		 $keyword = $this->requestEventClick();
		 /*菜单客服开始 2015年3月31日10:59:25*/
		 if($keyword=='客服女神'){
			$access_token = $this->new_getAccessToken();
			$url="https://api.weixin.qq.com/cgi-bin/customservice/getonlinekflist?access_token=".$access_token;
			$e=json_decode($this->httpGet($url),true);
			$user=array();
			for($i=0;$i<count($e['kf_online_list']);$i++){
				if($e['kf_online_list' ][$i]['status']>0 && $e['kf_online_list'][$i]['accepted_case']<3 )
					$user[]=$e['kf_online_list'][$i]['kf_account'];
			}
			if(!$user){
				$useraccount="zhuanjie@fangpinhui2014";
			}else{
				$useraccount=$user[array_rand($user)];
			}
            ...
            //$result  = sprintf($textTpl, $postObj->FromUserName, $postObj->ToUserName, time(),$useraccount);
			$result  = sprintf($textTpl, $this->getFromUserName(), $this->getToUserName(), time(),$useraccount);
            echo $result;
		 }
		 /*菜单客服结束 2015年3月31日10:59:25*/
		 
		 
		 
		 $sql="SELECT pid FROM fph_weixin_keyword_reply WHERE keyword='".$keyword."' AND status=1";
		 $res=mysql_query($sql);
		 $row=mysql_fetch_array($res,MYSQL_BOTH);
...
private function new_getAccessToken() {
    // access_token 应该全局存储与更新，我写入了文件中
...
/*新增加方法结束 2015年3月31日10:59:25*/

  /**
   * 数据库回复类型处理
   * @param fph_weixin_reply $id
   * @return string
   */
  
  public function replyType($id,$str){
  	if($id){ //如有关键字
  		$sql_w="SELECT pid,title,img,intro,url FROM fph_weixin_reply WHERE id=".$id." AND status=1 LIMIT 1";
  		$res_w=mysql_query($sql_w);
  		$row_w=mysql_fetch_array($res_w,MYSQL_BOTH);
  		switch ($row_w['pid']){
  			case 1://文字回复
				$title_reply = htmlspecialchars_decode($row_w['title']);
  				$this->responseTextMessage($title_reply);
  				break;
  			case 2://图片回复
  				$this->responseImageMessage("EA-e4Y9VDozjQVmscsKP-3LkAM0tg4y_sgQxOdByrbMPmeYyiGoVcAeYOQEciQAL");
  				break;
  			case 3://单图文回复
  				$title[0]['Title'] = "".$row_w['title']."";
  				$title[0]['PicUrl']= "http://image01.fangpinhui.com/".$row_w['img']."";
  				$title[0]['Description']   = "".$row_w['intro']."";
  				$title[0]['Url']   = "".$row_w['url']."&str=".$str;
  				$this->responseNewsMessage($title);
  				break;
  			case 4://多图文回复
  				$titlet= explode('//',$row_w['title']);
  				$img   = explode('@f&',$row_w['img']);
  				$url   = explode('@f&',$row_w['url']);
  				for($i = 0; $i < count($titlet); $i++){
  					$title[$i]['Title'] = "".$titlet[$i]."";
  					$title[$i]['PicUrl']= "http://image01.fangpinhui.com/".$img[$i]."";
  					$title[$i]['Url']   = "".$url[$i]."";
  				}
  				$this->responseNewsMessage($title);
  				break;
  		...
  		$this->responseTextMessage("亲，有什么问题可以直接去菜单栏找[客服女神]哦~女神在线时间：工作日9:00-18:00");
  	}
  }
  ...
  public function requestLocation() {
    ...
    $location['Scale']      = strval($this->postObj->Scale);//取出 缩放等级
    $location['Label']      = strval($this->postObj->Label);//取出 位置信息
  ...
  public function requestLink() {
	...
	$link['MsgId']       = strval($this->postObj->MsgId);
