...
class WeiXin {
  ...
  public function handleEvent(){
  	if($this->isEventSubscribe()){  //是否为普通关注事件
  		...
  		//用户信息写入到数据库
  		$search = "SELECT fromusername FROM fph_weixin_user WHERE fromusername = '".$fromUsername."'";
  		$query=mysql_query($search);
  		$rows = mysql_num_rows($query);   //几条
  		if(!$rows){       
  			$toUsername = $this->getToUserName();
  			$time       = Time();
  			$sql="insert into fph_weixin_user(fromusername,tousername,add_time) values(".$fromUsername.",".$toUsername.",".$time.")";
  			mysql_query($sql);
  		}	
